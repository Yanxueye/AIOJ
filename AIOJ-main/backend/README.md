...
Go + Gin + GORM + RabbitMQ + gRPC 在线判题平台后端。负责用户系统、题目管理、异步判题、学习统计、知识图谱、AI 能力转发等全部平台业务。
...
| 数据库 | `database/` | MySQL 初始化、AutoMigrate、种子数据（52 道题 + 标签字典） |
...
前端 POST /api/ai/{action}
    ↓
AIOJ backend 组装上下文（题目信息、提交历史、用户数据）
    ↓
HTTP POST → agent-service /api/agent/{action}
    ↓
agent-service 返回结构化 JSON
    ↓
AIOJ backend 透传给前端
```
...
**特殊：Solve full 级别状态机**
```
① 发送题目信息给 agent-service → ② 获取 AI 代码
    → ③ 调用判题验证 → ④ AC? 返回 : 回传错误重试（最多 3 轮）
```
...
- 学习计划（按知识点分组，含进度追踪）
- 签到打卡（每日统计）
- 知识图谱（73+ 节点，层级布局，掌握度着色）
- 每日推荐（基于薄弱知识点）
- 学习路径 / 薄弱分析

### AI 能力（经 agent-service）
- AI 对话（支持题目上下文注入、RAG 增强）
- 代码诊断（滑动窗口 3 次提交、判题结果注入）
- 解题辅助（hint / explain / full 三级，full 含沙箱验证重试）
- 知识图谱生成（基于用户做题数据，AI 分析掌握度）
- AI 端点限流（10/min/user）
...
| Favorite | 收藏 |
| AlgorithmTag / KnowledgePoint / ProblemKnowledgePoint | 统一标签体系 |
| UserKnowledgeGraph | 用户知识图谱（AI 生成） |
