...
func (h *Handler) GenerateSolution(c *gin.Context) {
	...
	if req.Problem != nil {
		if req.ProblemID == 0 { req.ProblemID = req.Problem.ID }
		if req.ProblemTitle == "" { req.ProblemTitle = req.Problem.Title }
		if req.ProblemDesc == "" { req.ProblemDesc = req.Problem.Content }
		if req.Editorial == "" { req.Editorial = req.Problem.Editorial }
		if len(req.AlgorithmTags) == 0 && len(req.Problem.Tags) > 0 { req.AlgorithmTags = req.Problem.Tags }
	}
	ragContext := ""
	if h.rag != nil && h.rag.IsInitialized() && len(req.AlgorithmTags) > 0 {
		ragContext = h.rag.BuildContext(strings.Join(req.AlgorithmTags, " "), 0)
	}
	systemPrompt := "你是题解撰写专家。基于用户通过的代码生成题解。用JSON格式回复。"
	userPrompt := fmt.Sprintf("题目: %s\n题目描述: %s", req.ProblemTitle, req.ProblemDesc)
	if req.Editorial != "" { userPrompt += fmt.Sprintf("\n\n官方题解:\n%s", req.Editorial) }
	if len(req.AlgorithmTags) > 0 { userPrompt += fmt.Sprintf("\n\n算法标签: %s", strings.Join(req.AlgorithmTags, ", ")) }
	if ragContext != "" { userPrompt += fmt.Sprintf("\n\n相关知识（从 OI-Wiki 检索）：\n---\n%s\n---", ragContext) }
	userPrompt += fmt.Sprintf("\n\n用户通过的代码（%s）:\n%s", req.Language, req.Code)
	userPrompt += fmt.Sprintf("\n\n候选算法标签：\n%s\n\n返回JSON格式：{title,content,algorithmTags,complexity:{time,space}}", CandidateTagDict)
	t0 := time.Now()
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"solution": "AI 服务暂时不可用", "provider": "unavailable"}})
		return
	}
	var structured map[string]interface{}
	if json.Unmarshal([]byte(resp), &structured) == nil {
		structured["rawMarkdown"] = resp; structured["provider"] = "agent-service"
		c.JSON(http.StatusOK, gin.H{"code": 0, "data": structured}); return
	}
	c.JSON(http.StatusOK, gin.H{"code": 0, "data": gin.H{"solution": resp, "rawMarkdown": resp, "provider": "agent-service"}})
}
...
func (h *Handler) CodeDiagnosis(c *gin.Context) {
	body, err := io.ReadAll(c.Request.Body)
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "failed to read body"}); return }
	var envelope PipelineEnvelope
	if err := json.Unmarshal(body, &envelope); err == nil && envelope.Task != "" {
		var req CodeDiagnosisPayload
		if err := json.Unmarshal(envelope.Payload, &req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "invalid payload"}); return
		}
		h.handleDiagnosis(c, &req); return
	}
	var req CodeDiagnosisPayload
	if err := json.Unmarshal(body, &req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "invalid request"}); return }
	h.handleDiagnosis(c, &req)
}

func (h *Handler) handleDiagnosis(c *gin.Context, req *CodeDiagnosisPayload) {
	mergeProblemFields(req)
	if req.Code == "" || req.Language == "" { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "code and language required"}); return }
	prompt := fmt.Sprintf("题目: %s\n题目描述: %s", req.ProblemTitle, req.ProblemDesc)
	if len(req.Samples) > 0 {
		prompt += "\n\n样例:\n"
		for i, s := range req.Samples { prompt += fmt.Sprintf("样例%d:\n  输入: %s\n  期望输出: %s\n", i+1, s.Input, s.Expected) }
	}
	if req.Editorial != "" { prompt += fmt.Sprintf("\n\n官方题解:\n%s", req.Editorial) }
	if len(req.RecentSubs) > 0 {
		prompt += "\n\n用户最近的提交记录:\n"
		for _, sub := range req.RecentSubs { prompt += fmt.Sprintf("- %s (%s): %s\n", sub.Status, sub.Language, sub.CreatedAt) }
	}
	...
	if req.JudgeStatus == "Accepted" { systemMsg = "你是算法竞赛教练。代码已通过全部测试，分析算法思路、复杂度和优化空间。用JSON格式回复。" }
	t0 := time.Now()
	...
	if err != nil { c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"summary": "AI 服务暂时不可用", "rawMarkdown": "AI 服务暂时不可用", "provider": "unavailable"}}); return }
	var structured map[string]interface{}
	if json.Unmarshal([]byte(resp), &structured) == nil { structured["rawMarkdown"] = resp; structured["provider"] = "agent-service"; c.JSON(http.StatusOK, gin.H{"code": 0, "data": structured}); return }
...
func mergeProblemFields(req *CodeDiagnosisPayload) {
	if req.Problem == nil { return }
	p := req.Problem
	if req.ProblemID == 0 { req.ProblemID = p.ID }
	if req.ProblemTitle == "" { req.ProblemTitle = p.Title }
	if req.ProblemDesc == "" { req.ProblemDesc = p.Content }
	if req.Editorial == "" { req.Editorial = p.Editorial }
	if len(req.Samples) == 0 && len(p.Samples) > 0 { req.Samples = p.Samples }
	if len(req.AlgorithmTags) == 0 && len(p.Tags) > 0 { req.AlgorithmTags = p.Tags }
}
...
func (h *Handler) KnowledgeGraph(c *gin.Context) {
	body, err := io.ReadAll(c.Request.Body)
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "failed to read body"}); return }
	var envelope PipelineEnvelope; var req KnowledgeGraphPayload
	if err := json.Unmarshal(body, &envelope); err == nil && envelope.Task != "" { json.Unmarshal(envelope.Payload, &req) } else { json.Unmarshal(body, &req) }
	prompt := "根据用户的做题记录分析知识掌握情况，生成知识图谱。\n\n"
	if len(req.Problems) > 0 {
		for _, p := range req.Problems { prompt += fmt.Sprintf("- %s (标签: %s, 状态: %s, 尝试: %d)\n", p.Title, strings.Join(p.Tags, ", "), p.Status, p.Attempts) }
	}
	if len(req.TagStats) > 0 {
		for tag, stats := range req.TagStats { prompt += fmt.Sprintf("- %s: 解决 %d/%d 题, 通过率 %.1f%%\n", tag, stats.Solved, stats.Attempted, stats.ACRate) }
	}
	...
	if err != nil { c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"nodes": []gin.H{}, "edges": []gin.H{}, "rawMarkdown": "AI 暂不可用", "provider": "unavailable"}}); return }
	var structured map[string]interface{}
	if json.Unmarshal([]byte(resp), &structured) == nil { structured["rawMarkdown"] = resp; structured["provider"] = "agent-service"; c.JSON(http.StatusOK, gin.H{"code": 0, "data": structured}); return }
...
func (h *Handler) CreateStudyPlan(c *gin.Context) {
	body, err := io.ReadAll(c.Request.Body)
	var envelope PipelineEnvelope; var req CreateStudyPlanPayload
	if json.Unmarshal(body, &envelope); err == nil && envelope.Task != "" { json.Unmarshal(envelope.Payload, &req) } else { json.Unmarshal(body, &req) }
	prompt := "根据用户的做题记录和薄弱知识点创建个性化学习题单。\n\n"
	if len(req.Problems) > 0 { for _, p := range req.Problems { prompt += fmt.Sprintf("- %s (标签: %s, 状态: %s)\n", p.Title, strings.Join(p.Tags, ", "), p.Status) } }
	if len(req.TagStats) > 0 { for tag, stats := range req.TagStats { prompt += fmt.Sprintf("- %s: 通过率 %.1f%%, 解决 %d/%d 题\n", tag, stats.ACRate, stats.Solved, stats.Attempted) } }
	if len(req.Candidates) > 0 { for tag, probs := range req.Candidates { prompt += fmt.Sprintf("\n【%s】:\n", tag); for _, p := range probs { prompt += fmt.Sprintf("  - #%d %s\n", p.ID, p.Title) } } }
	...
	if err != nil { c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"title": "默认题单", "description": "AI不可用", "problemIDs": []uint64{}, "rawMarkdown": "", "provider": "unavailable"}}); return }
	var structured map[string]interface{}
	if json.Unmarshal([]byte(resp), &structured) == nil { structured["rawMarkdown"] = resp; structured["provider"] = "agent-service"; c.JSON(http.StatusOK, gin.H{"code": 0, "data": structured}); return }
...
func (h *Handler) Solve(c *gin.Context) {
	...
	if err := json.Unmarshal(body, &envelope); err == nil && envelope.Task != "" {
		...
		if err := json.Unmarshal(envelope.Payload, &req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "invalid payload"}); return }
		h.handleSolve(c, &req); return
	}
	var req SolvePayload
	if err := json.Unmarshal(body, &req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "invalid request"}); return }
	h.handleSolve(c, &req)
...
func (h *Handler) handleSolve(c *gin.Context, req *SolvePayload) {
	...
	if req.EditorCode == "" && req.Code != "" { req.EditorCode = req.Code }
	level := req.Level; if level == "" { level = "hint" }
	contextInfo := fmt.Sprintf("题目: %s\n题目描述: %s", req.ProblemTitle, req.ProblemDesc)
	if len(req.Samples) > 0 { for i, s := range req.Samples { contextInfo += fmt.Sprintf("\n样例%d:\n  输入: %s\n  期望输出: %s\n", i+1, s.Input, s.Expected) } }
	if req.Editorial != "" { contextInfo += fmt.Sprintf("\n\n官方题解:\n%s", req.Editorial) }
	if req.EditorCode != "" { contextInfo += fmt.Sprintf("\n\n用户当前代码:\n%s", req.EditorCode) }
	if req.JudgeError != "" { contextInfo += fmt.Sprintf("\n\n之前的判题结果: %s", req.JudgeError) }
	var systemPrompt, userPrompt string
	switch level {
	case "hint":
		systemPrompt = "你是启发式算法教练。用旁敲侧击引导思考，不要给答案。用JSON格式回复。"
		if len(req.AlgorithmTags) > 0 { contextInfo += fmt.Sprintf("\n\n算法标签: %s", strings.Join(req.AlgorithmTags, ", ")) }
	...
	case "full":
		...
		if req.JudgeError != "" { userPrompt += fmt.Sprintf("\n之前的判题结果: %s\n请据此修正。", req.JudgeError) }
	default:
		systemPrompt = "你是算法教练。用JSON格式回复。"; userPrompt = contextInfo
	}
	resp, err := h.ai.Chat([]ai.Message{{Role: "system", Content: systemPrompt}, {Role: "user", Content: userPrompt}})
	log.Printf("[ai] solve LLM call took %v (level=%s)", time.Since(time.Now()), level)
	if err != nil { c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"answer": "AI 暂不可用", "provider": "unavailable"}}); return }
	var structured map[string]interface{}
	if json.Unmarshal([]byte(resp), &structured) == nil { structured["rawMarkdown"] = resp; structured["provider"] = "agent-service"; c.JSON(http.StatusOK, gin.H{"code": 0, "data": structured}); return }
...
func (h *Handler) UnifiedChat(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "invalid request"}); return }
	mode := req.Mode; if mode == "" { mode = "chat" }
	if !tools.IsValidMode(mode) { c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "unknown mode: " + mode}); return }
	...
	if len(toolDefs) > 0 {
		for round < maxRounds {
			...
			for i, td := range toolDefs { aiTools[i] = ai.ToolDefinition{Name: td.Name, Description: td.Description, Parameters: td.Schema} }
			result, err := h.ai.ChatWithTools(messages, aiTools, "auto")
			if err != nil { c.JSON(http.StatusOK, gin.H{"code": -1, "data": gin.H{"reply": "AI unavailable", "provider": "unavailable"}}); return }
			if len(result.ToolCalls) == 0 { break }
	...
	for _, m := range messages {
		if m.Role == "tool" { cleanMsgs = append(cleanMsgs, ai.Message{Role: "user", Content: m.Content}) } else { cleanMsgs = append(cleanMsgs, m) }
	}
...
func (h *Handler) buildModeMessages(req UnifiedChatRequest, mode string) []ai.Message {
	...
	if req.Problem != nil {
		...
		if len(req.Problem.Tags) > 0 { systemMsg += fmt.Sprintf("\n算法标签: %s", strings.Join(req.Problem.Tags, ", ")) }
		if req.Problem.Editorial != "" { systemMsg += fmt.Sprintf("\n官方题解: %s", req.Problem.Editorial) }
	}
	if len(req.ExtraProblems) > 0 {
		systemMsg += "\n\n关联题目："
		for _, ep := range req.ExtraProblems { systemMsg += fmt.Sprintf("\n- #%d %s (标签: %s)", ep.ID, ep.Title, strings.Join(ep.Tags, ", ")) }
	}
	if req.Code != "" {
		lang := req.Language; if lang == "" { lang = "代码" }
		systemMsg += fmt.Sprintf("\n\n用户代码（%s）:\n%s", lang, req.Code)
	}
...
func (h *Handler) Chat(c *gin.Context) {
	...
	if err := json.Unmarshal(body, &envelope); err == nil && envelope.Task != "" {
		...
		if err := json.Unmarshal(envelope.Payload, &payload); err == nil { req = h.chatPayloadToRequest(&payload) }
		...
		if len(req.Messages) == 0 {
			...
			if err := json.Unmarshal(envelope.Payload, &unified); err == nil && unified.Mode != "" {
				c.Request.Body = io.NopCloser(bytes.NewReader(envelope.Payload)); h.UnifiedChat(c); return
			}
		}
	}
	...
	if len(req.Messages) == 0 {
		...
		if err := json.Unmarshal(body, &unified); err == nil && unified.Mode != "" {
			c.Request.Body = io.NopCloser(bytes.NewReader(body)); h.UnifiedChat(c); return
		}
	}

	// 3. Legacy direct ChatRequest
	if len(req.Messages) == 0 {
		if err := json.Unmarshal(body, &req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid request"}); return }
	}
	if len(req.Messages) == 0 { c.JSON(http.StatusBadRequest, gin.H{"error": "messages required"}); return }

	systemContent := "你是一个算法竞赛AI助手，擅长算法、数据结构、编程竞赛相关问题。请用中文回答，回答简洁准确。"
	if h.rag != nil && h.rag.IsInitialized() {
		ragQuery := ""
		if len(req.Tags) > 0 { ragQuery = strings.Join(req.Tags, " ") }
		if ragQuery != "" {
			ragContext := h.rag.BuildContext(ragQuery, 2000)
			if ragContext != "" { systemContent += "\n\n相关知识（从 OI-Wiki 检索）：\n---\n" + ragContext + "\n---" }
		}
	}
	if req.Context != "" { systemContent += "\n\n以下是当前的上下文信息:\n" + req.Context }
	...
	if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": "AI 服务暂时不可用，请稍后重试"}); return }
...
func (h *Handler) chatPayloadToRequest(payload *ChatPayload) ChatRequest {
	...
	for _, m := range payload.History { if m.Content != "" { messages = append(messages, ai.Message{Role: m.Role, Content: m.Content}) } }
	if payload.Message != "" { messages = append(messages, ai.Message{Role: "user", Content: payload.Message}) }
	var contextParts []string
	if payload.Problem != nil {
		if payload.Problem.Title != "" { contextParts = append(contextParts, fmt.Sprintf("题目: %s", payload.Problem.Title)) }
		if payload.Problem.Content != "" { contextParts = append(contextParts, fmt.Sprintf("题面:\n%s", payload.Problem.Content)) }
		if len(payload.Problem.Samples) > 0 {
			sampleStr := "样例:\n"
			for i, s := range payload.Problem.Samples { sampleStr += fmt.Sprintf("样例%d:\n  输入: %s\n  期望输出: %s\n", i+1, s.Input, s.Expected) }
			contextParts = append(contextParts, sampleStr)
		}
		if len(payload.Problem.Tags) > 0 { contextParts = append(contextParts, fmt.Sprintf("标签: %s", strings.Join(payload.Problem.Tags, ", "))) }
	}
	if payload.Code != "" {
		lang := payload.CodeLanguage; if lang == "" { lang = "代码" }
		contextParts = append(contextParts, fmt.Sprintf("用户当前代码（%s）：\n%s", lang, payload.Code))
	}
	var tags []string
	if payload.Problem != nil { tags = payload.Problem.Tags }
